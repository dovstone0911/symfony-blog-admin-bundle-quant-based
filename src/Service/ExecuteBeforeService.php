...
{
    public function __construct(private Environment $twig, private PleaseService $please)
    {
        $this->please = $please;
        //
        $GLOBALS['Please'] = $please;
        $GLOBALS['MoSQLConfig'] = $this->please->setMoSQLConfig();
        $_SESSION['ENV'] = $_ENV;
        require_once dirname(__FILE__) . '/../Helpers/Quant.php';
        //
        $this->please->serve('cache')->cleanupExpiredFiles();
        $this->executeBefore();
    }

    public function executeBefore()
    {
        $this->twig->addGlobal('User', $this->please->serve('security')->getFreshUser());
        // $this->setVisitsCount();
    }
    ...
    private function setVisitsCount()
    {
        ...
        */
        ...
        $this->please->unsetGlobal(['online', 'day', 'yesterday', 'week', 'month', 'year', 'all']);
        $this->please->setGlobal([
            'online' => $online,
            'day' => $day,
            'yesterday' => $yesterday,
            'week' => $week,
    ...
    public function defaultFallback($key)
    {
        // $hookFallbacks = $this->hookFallbacks();
        // if (isset($hookFallbacks[$key]) && is_callable($hookFallbacks[$key])) {
        //     return $hookFallbacks[$key]();
        // }
        // //force redirection
        // return $this->please->redirectToReferer();
    }

    // private function hookFallbacks()
    // {
    //     if ($this->please->prevContainer->has('hook') && method_exists($defaults = $this->please->prevContainer->get('hook'), 'hookFallbacks')) {
    //         return $defaults->hookFallbacks();
    //     }
    //     //force redirection
    //     return $this->please->redirectToReferer();
    // }
