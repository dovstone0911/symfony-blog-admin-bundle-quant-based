    ...
    private function processAssetArray(array $paths, string $extension, $reset = false): string
    {
        ...
        foreach ($paths as $i => $path) {
            ...
            if ($i === $pathCount - 1) {
                ...
                $this->log->reWrite('assets', ['filename' => $path, 'where'=>2, 'mixedFilePath' => $mixedFilePath, 'content' => $processedContent], $executionTime);
                return $this->getAssetFullContentTag($mixedKey, $extension, $paths);
            }
        }
        ...
        return $this->cache->do(function () use ($paths, $extension, $isCDN) {
            ...
            foreach ($paths as $i => $path) {
                ...
                if ($i === $pathCount - 1) {
                    ...
                    file_put_contents($mixedFilePath, $processedContent, LOCK_EX);
                    $executionTime = microtime(true) - $startTime;
                    $this->log->reWrite('assets', ['filename' => $path, 'where'=>1, 'mixedFilePath' => $mixedFilePath, 'content' => $processedContent], $executionTime);
                    return $this->getAssetFullContentTag($mixedKey, $extension, $paths);
                }
            }
