...
class CRUDService extends AbstractController
{
        ...
        try {
            ...
            if (
                ...
                return $this->checkDataValidity($params, function ($params) {
                    ...
                    $data = quant($coll_name)->insert($data);
                    //
    ...
    public function readList($params)
    {
        ...
        try {
            ...
            $finderCriteria = is_callable($params['finderCriteria']) ? $params['finderCriteria'](quant($coll_name)) : $params['finderCriteria'];
            ...
            $query = quant($coll_name)->whereBy($finalCriteria)->orderBy($finderCriteria[1] ?? []);
            ...
            $total = $query->count();
            $documents = $query->limit($params['limit'])->fetch();
        ...
        try {
            ...
            if (
                ...
                return $this->checkDataValidity($params, function ($params) use ($document) {
                    ...
                    $data = quant($coll_name)->where('id', $document['id'])->update($data);
    ...
    public function delete($params)
    {
        ...
        try {
            ...
            $document = $params['finder'](quant($coll_name));
            ...
            foreach ($document as $doc) {
                quant($coll_name)->delete(['id' => attr($doc, 'id')]);
                if ($coll_name == 'file') {
    ...
    public function upsert($params)
    {
        ...
        try {
            ...
            $document = quant($coll_name)->upsert(
                $keysReferences,
    ...
    private function _deleteCache($data): void
    {
        ...
        if ($user_id = $this->please->serve('security')->getUserId()) {
            quant('users')->where('id', $user_id)->update([
                'updated_at' => (new \DateTime)->format('Y-m-d H:i:s')
            ]);
        }

        // lets basic update _touch_ids
        $touch_ids = input('_touch_ids', []);
        foreach ($touch_ids as $coll_name => $id) {
            quant($coll_name)->where('id', $id)->update([
                'updated_at' => (new \DateTime)->format('Y-m-d H:i:s')
            ]);
        }
    }
