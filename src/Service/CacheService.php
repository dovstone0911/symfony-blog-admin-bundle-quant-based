...
class CacheService extends AbstractController
{
    ...
    public function session(callable $onSuccess, $cacheKey, $ttl = "1 hour")
    {
    ...
    public function getLastUpdatedAt(array|string|null $params): ?string
    {
        $sql_cache = $this->please->serve('env')->getAppEnv('SQL_CACHE') === "true";
        ...
        if (is_string($params)) {
            $watchCollections = [$params];
        } elseif (is_array($params)) {
            if (!empty($params['watchCollections']) || !empty($params['watch_collections'])) {
                $watchCollections = array_merge($params['watchCollections'] ?? [], $params['watch_collections'] ?? []);
            } elseif (!empty($params['coll_name'])) {
                $watchCollections = [$params['coll_name']];
            }
        }

        if (empty($watchCollections)) {
            return null;
        }

        $queries = array_map(
            fn ($c) => "SELECT MAX(updated_at) FROM " . preg_replace('/[^a-zA-Z0-9_]/', '_', $c),
            $watchCollections
        );

        $result = qb()->rawQuery(implode(" UNION ALL ", $queries));
        $max = max(array_column($result, 'MAX(updated_at)'));
    ...
    private function doMemoryCache(callable $onSuccess, $cacheKey, $ttl = "1 hour", $cache)
    {
    ...
    private function doFileCache(callable $onSuccess, $cacheKey, $ttl = "1 hour")
    {
        $timeServ = $this->please->serve('time');
        $cacheDir = $this->getCacheDir();
        $cachefile = "$cacheDir/$cacheKey.cache";
    ...
    {
        // $DS = DIRECTORY_SEPARATOR;
        //$dirPath = dirname(dirname(dirname(__FILE__))) . $DS . 'cache';
