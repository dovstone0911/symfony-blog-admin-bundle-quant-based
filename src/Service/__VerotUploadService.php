...
class __VerotUploadService
{
    ...
    function getsize($size)
    {
        ...
        preg_match_all('/(.*)(G|M|K)/mi', $size, $matches, PREG_SET_ORDER, 0);
        $last = strtolower($matches[0][2] ?? 'g');
        $size = (float)$size;
        switch ($last) {
            case 'g':
                $size *= 1024;
                ...
                if ($this->gdversion()) {
                    switch ($this->image_src_type) {
                        ...
                        case 'webp':
                            ...
                            break;
                        case 'wbmp':
                            $image_src = @imagecreatefromwbmp($this->file_src_pathname);
                            if (!$image_src) {
                                $this->processed = false;
                                $this->error = $this->translate('create_error', array('WBMP'));
                            } else {
                                $this->log .= '- source image is wbmp<br />';
                            }
                            break;
                        case 'xbm':
                            $image_src = @imagecreatefromxbm($this->file_src_pathname);
                            if (!$image_src) {
                                $this->processed = false;
                                $this->error = $this->translate('create_error', array('XBM'));
                            } else {
                                $this->log .= '- source image is xbm<br />';
                            }
                            break;
                        default:
                            $this->processed = false;
